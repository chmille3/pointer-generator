...
class SummarizationModel(object):
  ...
  def _reduce_states(self, fw_st, bw_st):
    ...
    with tf.variable_scope('reduce_final_st'):
      ...
      w_reduce_c = tf.get_variable('w_reduce_c', [hidden_dim.value * 2, hidden_dim.value], dtype=tf.float32, initializer=self.trunc_norm_init)
      w_reduce_h = tf.get_variable('w_reduce_h', [hidden_dim.value * 2, hidden_dim.value], dtype=tf.float32, initializer=self.trunc_norm_init)
      bias_reduce_c = tf.get_variable('bias_reduce_c', [hidden_dim.value], dtype=tf.float32, initializer=self.trunc_norm_init)
      bias_reduce_h = tf.get_variable('bias_reduce_h', [hidden_dim.value], dtype=tf.float32, initializer=self.trunc_norm_init)
